		...
		if($this->show_rc_numbers && !$this->flags->never_show_order) {
			...
			$output->insertOrDeleteColumns($target, $output->get_width() + 1);
			{
				...
				while($_g11 < $_g2) {
					...
					$output->setCell(0, $i1, $this->builder->links($unit));
					unset($unit,$i1);
				}
			}
			$target = new _hx_array(array());
			...
			$output->insertOrDeleteRows($target, $output->get_height() + 1);
			{
				...
				while($_g13 < $_g4) {
					...
					$output->setCell($i3, 0, $this->builder->links($unit1));
					unset($unit1,$i3);
				}
			}
			...
			while($_g1 < $_g) {
				...
				if($have_pp) {
					if(!$have_rr) {
						...
						if($this->v->equals($pp, $rr)) {
							$dd = $pp;
						} else {
							$dd = $pp;
							$dd_to = $rr;
							$have_dd_to = true;
							if(!$this->v->equals($pp, $ll)) {
								if(!$this->v->equals($pp, $rr)) {
									$dd_to_alt = $ll;
									$have_dd_to_alt = true;
								}
				...
				if($have_dd_to && $this->allow_update) {
					...
					$is_conflict = false;
					if($have_dd_to_alt) {
						if(!$this->v->equals($dd_to, $dd_to_alt)) {
							$is_conflict = true;
						}
					}
					if(!$is_conflict) {
