	...
	public function alignCore2($align, $a, $b) {
		...
		if($ids !== null) {
			...
			if($common_units->length > $N) {
				...
				$sorter = array(new _hx_lambda(array(), "coopy_CompareTable_0"), 'execute');
				$columns_eval->sort($sorter);
				$columns = Lambda::harray(Lambda::map($columns_eval, array(new _hx_lambda(array(), "coopy_CompareTable_1"), 'execute')));
...
function coopy_CompareTable_0($a1, $b1) {
	{
		if($a1->a[1] < $b1[1]) {
			return 1;
		}
...
function coopy_CompareTable_1($v) {
	{
		return $v[0];
	}
}
