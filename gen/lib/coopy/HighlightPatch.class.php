...
class coopy_HighlightPatch implements coopy_Row{
					...
					if($cmod->add) {
						{
							...
							while($_g21 < $_g31->length) {
								$mod2 = $_g31[$_g21];
								++$_g21;
								if($mod2->patchRow !== -1 && $mod2->destRow !== -1) {
									$d1 = $this->patch->getCell($cmod->patchRow, $mod2->patchRow);
									$this->source->setCell($cmod->destRow, $mod2->destRow, $d1);
									unset($d1);
								}
								unset($mod2);
							}
	...
	public function finishColumns() {
		...
		$inc = array(new _hx_lambda(array(), "coopy_HighlightPatch_0"), 'execute');
		{
			$_g13 = 0;
			$_g4 = $fate->length;
			while($_g13 < $_g4) {
...
function coopy_HighlightPatch_0($x) {
	{
		if($x < 0) {
			return $x;
		} else {
